...
async function markUnverifiedAsAbsentUniversityWide(sessionId) {
    try {
        ...
        if (!sid) {
            console.error('markUnverifiedAsAbsentUniversityWide: invalid sessionId');
            return;
        }
        ...
        if (unverified.length === 0) {
            console.log(`✅ All students verified for session ${sid}`);
        ...
        if (existingError) {
            console.error('Error fetching existing scan rows:', existingError);
        }
        ...
        if (toInsert.length > 0) {
            ...
            for (let i = 0; i < absentInserts.length; i += CHUNK_SIZE) {
                const chunk = absentInserts.slice(i, i + CHUNK_SIZE);
                const { error: insertError } = await supabase
                    .from('bedcheck_scans')
                    .insert(chunk);

                if (insertError) {
                    console.error(`Error inserting absent scans chunk ${i}:`, insertError);
                }
            }
        ...
        console.error('Error in markUnverifiedAsAbsentUniversityWide:', error);
    }
}
    ...
    async (req, res) => {
        try {
            ...
            if (!verificationResult.success || !verificationResult.verified) {
                return res.status(400).json({
                    success: false,
                    message: verificationResult.error || 'Face verification failed - face does not match',
...
app.get('/api/rooms',
    ...
    async (req, res) => {
        ...
        try {
            ...
            let hostelQuery = supabase
                .from('hostels')
                .select('id, type, name, campus');

            // Non-admin users can only access their campus
            if (!isAdmin) {
                hostelQuery = hostelQuery.eq('campus', req.campus);

                // If the user has a hostel assignment,
                // restrict them to that hostel.
                if (req.user.hostel_id) {
                    hostelQuery = hostelQuery.eq(
                        'id',
                        parseInt(req.user.hostel_id)
                    );
                }
            }

            // Optional hostel filter
            if (hostel_id) {
                hostelQuery = hostelQuery.eq(
                    'id',
                    parseInt(hostel_id)
            ...
            if (hostelError) {
                console.error(
                    '❌ Error fetching hostels for rooms:',
                    hostelError
                );
                throw hostelError;
            }
            ...
            if (floor_flat_id) {
                floorsQuery = floorsQuery.eq(
                    'id',
                    parseInt(floor_flat_id)
                );
            }
            ...
            /*
             * bed_spaces DOES have campus,
             * so campus filtering is valid here.
             */
            if (!isAdmin) {
                bedsQuery = bedsQuery.eq(
                    'campus',
                    req.campus
                );
            }
            ...
            const enriched = rooms.map(room => {
                ...
                const floor =
                    floorMap[room.floor_flat_id] || {};

                const beds =
                    bedsByRoom[room.id] || [];

                /*
                 * Count occupied beds from actual bed_spaces
                 */
                ...
                /*
                 * Prefer actual bed count.
                 * If no bed records exist, use room.capacity.
                 */
                const capacity =
                    beds.length > 0
                        ? beds.length
...
app.post('/api/bedcheck/scans',
    ...
    async (req, res) => {
        try {
            const { session_id, student_id, room, bed_number, status, metadata } = req.body;
            ...
            if (student_id) {
                ...
                let studentQuery = supabase
                    .from('students')
                    .select('id, name, matric, hostel_id, room_id, room_code, bed_code, bed_space_id, campus')
            ...
            const now = new Date().toISOString();
            const globalSessionId = parseInt(session_id);
            const attendanceStatus =
                (status === 'Verified' || status === 'Present') ? 'Present' : (status || 'Present');

            let resolvedBed = bed_number || student?.bed_code || null;

            if (!resolvedBed && student?.bed_space_id) {
                try {
                    const { data: bed } = await supabase
                        .from('bed_spaces')
                        .select('bed_code, full_bed_code')
                        .eq('id', student.bed_space_id)
                        .maybeSingle();

                    resolvedBed = bed?.full_bed_code || bed?.bed_code || null;
                } catch (_) {
                }
            } else if (!resolvedBed && student?.room_id && student?.id) {
                try {
                    const { data: bed } = await supabase
                        .from('bed_spaces')
                        .select('bed_code, full_bed_code')
            ...
            const newScan = {
                ...
                room: room || student?.room_code || null,
                bed_number: resolvedBed,
            ...
            await auditService.log({
                ...
                action: 'Face Verification',
                module: 'verification',
                details: `${student?.name || 'Unknown'} verified in ${room || student?.room_code || 'Unknown Room'}`,
                result: 'success',
